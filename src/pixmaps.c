...
gboolean
create_pixmaps (GtkWidget * widget, gint first_time)
{
  gint i, j;
...
/* allocating memory for pixmaps */
  if (first_time) {
    ...
    border_width_xpm[2] = (gchar *) g_malloc (MAX_CELL_SIZE * X_SIZE +
					      2 * border + 1);
...
/* border pixmaps */
  ...
  border_w_gdkxpm =
    gdk_pixbuf_new_from_xpm_data ((const char **) &border_width_xpm);
  if (border_h_gdkxpm)
    g_object_unref (border_h_gdkxpm);
  border_h_gdkxpm =
    gdk_pixbuf_new_from_xpm_data ((const char **) &border_height_xpm);
...
redraw_cells ()
{
  ...
  for (j = 0; j < Y_SIZE; j++) {
	...
	gdk_draw_pixbuf (drawingarea->window, drawingarea->
			 style->fg_gc[GTK_WIDGET_STATE (drawingarea)],
			 colors[cell_state_],
			 0, 0, border + i * cell_width,
...
redraw_all_cells ()
{
  ...
  for (i = 0; i < 2; i++)
    gdk_draw_pixbuf (drawingarea->window,
		     drawingarea->style->
		     fg_gc[GTK_WIDGET_STATE (drawingarea)], border_w_gdkxpm,
		     0, 0, 0, i * (border + Y_SIZE * cell_width),
		     cell_width * X_SIZE + 2 * border, border,
		     GDK_RGB_DITHER_NONE, 0, 0);
  for (i = 0; i < 2; i++)
    gdk_draw_pixbuf (drawingarea->window,
		     drawingarea->style->
		     fg_gc[GTK_WIDGET_STATE (drawingarea)], border_h_gdkxpm,
		     0, 0, i * (X_SIZE * cell_width + border),
		     border, border, cell_width * Y_SIZE,
		     GDK_RGB_DITHER_NONE, 0, 0);
  for (j = 0; j < Y_SIZE; j++) {
    for (i = 0; i < X_SIZE; i++) {
      gdk_draw_pixbuf (drawingarea->window,
		       ...
		       colors[cell_state[i][j]], 0, 0, border + i * cell_width,
